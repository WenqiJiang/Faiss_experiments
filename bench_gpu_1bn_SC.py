...
def eval_dataset_from_dict(target_DB=None, target_index=None, target_topK=None, target_recall_goal=None):
    ...
    for dbname in d_nprobes:
        ...
        if target_DB is not None and dbname != target_DB:
            continue
        if dbname.startswith('SIFT'):
            # SIFT1M to SIFT1000M
            global dbsize
        ...
        for index_key in d_nprobes[dbname]:
            ...
            if target_index is not None and index_key != target_index:
                continue
            ...
            for topK in d_nprobes[dbname][index_key]:
                ...
                if target_topK is not None and topK != target_topK:
                    continue
                ...
                for recall_goal in d_nprobes[dbname][index_key][topK]:
                    ...
                    if target_recall_goal is not None and recall_goal != target_recall_goal:
                        continue
...
if load_from_dict:  # usage 2
    eval_dataset_from_dict(target_DB=dbname, target_index=index_key, target_topK=topK, target_recall_goal=recall_goal)
elif recall_goal: # usage 3
    preproc = get_preprocessor()
    index = get_populated_index(preproc)
