...
if dbname.startswith('SIFT'):
    ...
    gt_I = np.array(gt_list)
    gt_I = np.reshape(gt_I, (gt_I.shape[0] * gt_I.shape[1], -1))
...
def eval_dataset(index, preproc):
    ...
    print("query_batch_size: ", sl)
    nq = xq.shape[0]
    print("xq_shape: ", xq.shape)
    print("gt_shape: ", gt_I.shape)
    ...
    for nprobe in nprobes:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            conn, addr = s.accept()
            t0 = time.time()
            #I = b'T8\x8a\x05\x9b>A\x00\xa06I\x01X\xc5\xcc\x00\x83$Q\x02EQ.\x00,\xd2\xf2\x01/\x97\xaf\x00\x87\xcd\xd8\x05\n@n\x03' 
            with conn:
                print('Connected by', addr)
                for i in range(QUERY_NUM):
                    rq = conn.recv(BYTES_PER_QUERY)
                    query_vec = np.fromstring(rq, dtype=np.float32)
                    ...
                    I = I.tostring()
                    #print("Query {}, server compute time = {} ms".format(i, 1000*(tt1-tt0)))
                    conn.sendall(I)
        
            t1 = time.time()
            print("Server QPS = {} (using a single client)".format(QUERY_NUM / (t1 - t0)))
